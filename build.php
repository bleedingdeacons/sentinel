...
/**
 * Build Script for Sentinel WordPress Plugin
 *
...
class PluginBuilder
{
    ...
    private string $pluginName = 'sentinel';
    ...
    private array $productionExcludes = [
        ...
        // Working vendor/ is excluded wholesale; stageProductionVendor()
        // stages a clean `composer install --no-dev` copy and injects that
        // instead, so production dependencies (psr/container) still ship but
        // the dev test tooling never does.
            'vendor',
    ...
    private function getVersionFromPlugin(): string
    {
        ...
        if (file_exists($mainFile)) {
            ...
            if (preg_match("/define\s*\(\s*'SENTINEL_VERSION'\s*,\s*'([^']+)'/", $content, $matches)) {
                return $matches[1];
            }
        }
        return '0.0.1';
    ...
    private function createZip(string $archiveName, array $excludes, ?string $stagedVendor = null): void
    {
        ...
        foreach ($files as $file) {
            ...
            // Add to ZIP with plugin directory prefix
...
Sentinel WordPress Plugin Build Script
==========================================
Platform: {$platform}
...
Note:
  Sentinel is a monitoring widget for the Integrity plugin. No additional
  dependencies beyond Integrity are required — just install and activate
  to see Integrity status on your WordPress dashboard.
